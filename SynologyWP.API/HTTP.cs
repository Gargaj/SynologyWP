...
namespace SynologyWP.API
{
  public class HTTP
  {
      ...
      try
      {
        ...
        else if (method == "POST")
        {
          ...
          if (headers["Content-Type"] != null)
          {
            ...
            {
              var meta = types[i].Trim().Split('=');
              if (meta.Length == 2)
              {
                if (meta[0] == "charset")
                {
                  content.Headers.ContentType.CharSet = meta[1];
                }
              }
            }
          }
    ...
    {
      if (_params != null && _params.AllKeys.Any())
      {
        _url += "?" + string.Join("&", _params.AllKeys.Select(s => $"{s}={WebUtility.UrlEncode(_params.GetValues(s)[0])}"));
      }
